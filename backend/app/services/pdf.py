...
def extract_text_from_pdf(pdf_source) -> str:
    """Extract text from PDF (handles file path, bytes, or BytesIO)."""
    if isinstance(pdf_source, (str, Path)):
        return extract_text_from_file(Path(pdf_source))
    elif isinstance(pdf_source, bytes):
        return extract_text_from_bytes(pdf_source)
    elif hasattr(pdf_source, 'read'):
        # BytesIO or file-like object
        return extract_text_from_bytes(pdf_source.read())
    else:

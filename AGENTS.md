...
| Vercel | `POST /api/pdf/upload/[id]` complete | HEAD + enqueue `upload.extract` immediately (SDK then REST; **503** if no run id). Records `extract_started_at`. `GET` re-nudges `uploaded` only if that stamp is missing or older than 15s. `upload.drain` is the orphan net. |

...
| `frozen-script.ts` | First take-home claim writes `speakable.txt` + `sections.json`. Before that, one DeepSeek listen-prep call (`listen-prep.ts`) sees the file title and the front of the text — not a guess at which published book it is — and returns lines to leave unspoken (copyright, ISBN) plus headings to speak alone. A short foreword stays. The prose is not rewritten. A miss keeps the existing cleanup. Then the cue-tag pass and pack for Fish / Edge / Google. Fish / clone even-packs to fan-out; Google packs against SSML bytes (`packProvider: "google"`); later ticks never re-split or re-download the book |

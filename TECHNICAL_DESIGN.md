...
| `split-text.ts` → `packSpeakableSections` | Chapter-aware paragraph packer; optional `measure` (Google = SSML UTF-8 bytes); page-number lines are layout, not speech boundaries. A heading main would keep is kept. A contents stub loses when a later opening of the same number has a much longer body. A sentence citation is not a heading, and a bare label continued by "of" or "is" is not one either. Number order only chooses between those duplicates. Another Chapter 1 with a real body starts a new sequence, including after Book the First / Book the Second. |

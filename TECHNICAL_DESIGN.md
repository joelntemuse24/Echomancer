...
| Function | Role |
|----------|------|
| `listCatalogVoices(filters)` | Slim catalog: Standard, Randolph; clones merged in voices API |
...
| Synth path | Standard → `edgeTtsProvider`. Randolph → `googleTtsProvider` (`en-GB-Neural2-O`). Curated Fish stock (when listed) and user clones → `fishTtsProvider` with `reference_id` when `FISH_API_KEY` is set. Legacy `fish-narrator`: same Fish endpoint **without** `reference_id`. Never send OpenRouter catalog UUIDs as `reference_id`. |
...
(Standard, Randolph + session clones).
...
// Edge stock (standard) → edge adapter
// Curated Fish stock (when listed) → fish adapter (with reference_id)
...
**Standard** path. Talks to Microsoft Edge’s undocumented Read Aloud
...
subscription. Output is MP3. Voice id is `en-US-AndrewNeural`.
...
browser exposes Andrew Neural. Otherwise the picker falls back to
`POST /api/tts/preview`. Never picks a random system voice. Randolph always
uses Google Cloud TTS via preview / the job worker.

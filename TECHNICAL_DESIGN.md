...
| `listen-prep.ts` | Whole-book cleanup once per upload. A deterministic pre-pass drops sequential page numbers, repeated running headers (blank lines do not hide the neighboring page number; a header must sit at page boundaries on at least five pages, and a speaker label followed by dialogue is kept; the book's first line is kept once by exact text next to a page number, unless that line is a chapter heading), and Gutenberg boilerplate. Header repeats use a frequency map. Chunks of about 8k tokens then go to `LISTEN_PREP_MODEL` (default `google/gemini-3.8-flash`, minimal reasoning, strict JSON schema, AI Studio then Vertex, 20s, one retry on 429/5xx). A bad JSON or schema reply uses `LISTEN_PREP_FALLBACK_MODEL` (DeepSeek via Together then DeepInfra). The guard restores protected reading lines and headings, and keeps index, contents, notes, and bibliography drops. A chunk whose drop share exceeds `LISTEN_PREP_MAX_DROP_SHARE` is kept unless most lines are page citations, bibliography or note lines, or other clutter. Short title-case lines do not qualify. Chapter and part headings stay even when the model does not mark them. A failed or fallback chunk is stored as partial and retried on a later pass, up to three attempts. Freeze uses a cleaned file for this source immediately. With no best-so-far it waits up to `LISTEN_PREP_PASS_WAIT_MS` (default 45s) for the other pass, then cleans the book itself, and does not persist raw text. A running record does not reuse a cleaned file from an older source hash. Eight chunks per book, about 20 requests in flight on the worker. |
| `frozen-script.ts` | First take-home claim writes `speakable.txt`, `sections.json`, and a small `playback-chapters.json`. It reuses `pdfs/<uploadId>/listen-cleaned.txt` when that record is settled. A partial clean for this source is used immediately and a retry is scheduled without blocking the freeze. With no cleaned file, freeze waits for the running pass, then cleans the book itself. Then pack for Fish / Edge / Google. Fish synthesis text has no square-bracket cues. Fish / clone even-packs to fan-out; Google packs against SSML bytes (`packProvider: "google"`); later ticks never re-split or re-clean |

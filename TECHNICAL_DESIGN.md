...
| `split-text.ts` → `packSpeakableSections` | Chapter-aware paragraph packer; optional `measure` (Google = SSML UTF-8 bytes); page-number lines are layout, not speech boundaries. A sentence that only cites "Chapter 3" is not a heading. The same chapter number, or a lower number after a later chapter, stays inside the current chapter. A part resets chapter numbering. |

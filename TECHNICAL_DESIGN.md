...
| `split-text.ts` → `packSpeakableSections` | Chapter-aware paragraph packer; optional `measure` (Google = SSML UTF-8 bytes); page-number lines are layout, not speech boundaries. Headings are main's outline. A line is removed only as a contents run (three or more headings in a row, each body under 200 characters, with a later same number or title that has a real body), a Notes/Endnotes/References copy of the same number and title, a numbered intro sentence before the first real body, a bare chapter label inside an Index, or a repeated running head beside a page number. Any Chapter 1 with a body is kept. A same-line "Chapter 3 shows…" stays prose. |

...
| `split-text.ts` → `packSpeakableSections` | Chapter-aware paragraph packer; optional `measure` (Google = SSML UTF-8 bytes); page-number lines are layout, not speech boundaries. A chapter line over 80 characters, a comma- or quote-led summary, or a line whose next block starts lowercase is not a heading. A contents entry loses to the later opening of the same number when it has almost no body. Book, Volume, Part, and a title directly before Chapter 1 start numbering over. The next number after the last in-sequence chapter still opens, even if an outlier raised the high-water mark. |

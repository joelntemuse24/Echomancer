...
| `listen-prep.ts` | Whole-book cleanup before freeze, for every reader. Chunks of about 8k tokens (hard cap 10k) run in parallel (`LISTEN_PREP_CONCURRENCY`, default 8) through OpenRouter DeepSeek Flash (`LISTEN_PREP_MODEL`). The model returns line ids to drop and heading ids. Kept lines are the original bytes. A timeout, bad JSON, or a drop of more than 40% of a prose chunk leaves that chunk unchanged. Front and back matter may drop more. Dedications and epigraphs stay. |

...
| `listen-prep.ts` | Whole-book cleanup once per upload. A deterministic pre-pass drops sequential page numbers, Gutenberg boilerplate, and a running header only when the same text sits at the top of at least five pages, is at most 60 characters, has no sentence punctuation, comma, or quote, is not a speaker label, and is not the book's first line. A model drop may also remove a header that appears at least twice under those same exclusions. After a Contents label, a model drop may remove short lines that are not sentences, quotes, or speaker labels, until prose or a repeated entry title, plus an unlabeled run of three or more contents-shaped lines. Those guesses do not lift the 40% cap. A DeepSeek fallback reply does not receive that extra allowance. Header repeats use a frequency map. Chunks of about 8k tokens then go to `LISTEN_PREP_MODEL` (default `google/gemini-3.8-flash`, minimal reasoning, strict JSON schema, AI Studio then Vertex, 20s, one retry on 429/5xx). A bad JSON or schema reply uses `LISTEN_PREP_FALLBACK_MODEL` (DeepSeek via Together then DeepInfra). The guard restores protected reading lines and headings, and keeps index, contents, notes, and bibliography drops. A chunk whose drop share exceeds `LISTEN_PREP_MAX_DROP_SHARE` is kept unless most lines are page citations, bibliography or note lines, or other clutter. Header and contents guesses do not lift that cap. A heading is kept when it appears once beside prose. Short title-case lines do not qualify. A failed or fallback chunk is stored as partial and retried on a later pass, up to three attempts. Freeze uses a cleaned file for this source immediately. With no best-so-far it waits up to `LISTEN_PREP_PASS_WAIT_MS` (default 45s) for the other pass. If the tick cannot fit a full model pass it requeues instead of freezing a skipped or truncated clean, and it does not write a running record for that skip. It does not persist raw text. A running record does not reuse a cleaned file from an older source hash. Eight chunks per book, about 20 requests in flight on the worker. |

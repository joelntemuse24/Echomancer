...
| `listen-prep.ts` | Whole-book cleanup once per upload. Chunks of about 8k tokens run in parallel through OpenRouter (`LISTEN_PREP_MODEL`). The model returns line ids to drop, heading ids, and a short note. Kept lines are the original bytes. Drops of reading lines are limited to the contiguous labels before the body and after it. Every chunk stays under a 40% character cap and is never emptied. A timeout or bad JSON leaves that chunk unchanged. |

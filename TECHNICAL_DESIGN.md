...
| `listen-prep.ts` | Whole-book cleanup once per upload. A deterministic pre-pass drops sequential page numbers, repeated running headers (blank lines do not hide the neighboring page number; the book's first line is kept once by exact text next to a page number), and Gutenberg boilerplate. Header repeats use a frequency map. Chunks of about 8k tokens then go to `LISTEN_PREP_MODEL` (default `google/gemini-3.8-flash`, minimal reasoning, strict JSON schema, AI Studio then Vertex, 20s, one retry on 429/5xx). A bad JSON or schema reply uses `LISTEN_PREP_FALLBACK_MODEL` (DeepSeek via Together then DeepInfra). The guard restores protected reading lines and headings, and keeps index, contents, notes, and bibliography drops. A chunk whose drop share exceeds `LISTEN_PREP_MAX_DROP_SHARE` and is not index, contents, or notes is kept. A failed or fallback chunk is stored as partial and retried on a later pass, up to three attempts. Freeze reads the best text so far and does not wait on a retry. Eight chunks per book, about 20 requests in flight on the worker. |
| `frozen-script.ts` | First take-home claim writes `speakable.txt`, `sections.json`, and a small `playback-chapters.json`. It reuses `pdfs/<uploadId>/listen-cleaned.txt` when that record is settled. A partial clean is used immediately and a retry is scheduled without blocking the freeze. Then pack for Fish / Edge / Google. Fish synthesis text has no square-bracket cues. Fish / clone even-packs to fan-out; Google packs against SSML bytes (`packProvider: "google"`); later ticks never re-split or re-clean |

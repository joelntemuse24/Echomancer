...
| `split-text.ts` → `packSpeakableSections` | Chapter-aware paragraph packer; optional `measure` (Google = SSML UTF-8 bytes); page-number lines are layout, not speech boundaries. A heading main would keep is kept. It is removed only as a contents stub (the earlier body is under 80 characters), a comma- or quote-led citation, a bare label continued by a lowercase "of" or "is", a running head beside a page number, or a copy after Notes, Endnotes, Appendix, Bonus, Index, or References. Another Chapter 1 starts a sequence only when a title, Book, Part, or Volume sits between the two and the gap is more than a page. |
